...
func Example() {
	...
	signer, err := agentwallet.NewBIP44Signer(seed, agentwallet.DefaultCoinType, 42)
	if err != nil {
		panic(err)
	}
	...
	params := agentwallet.Permit2TransferParams{
		...
		ChainID:     8453,
		Permit2Addr: agentwallet.CanonicalPermit2Address,
	}
	sp, _ := signer.SignPermit2(ctx, params)

	td, _ := agentwallet.BuildPermit2TypedData(params)
	recovered, _ := agentwallet.RecoverTypedDataSigner(td, sp.Signature)
